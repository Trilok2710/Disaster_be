...
// GET /resources/:disasterId (public)
router.get('/:disasterId', async (req, res) => {
  const { disasterId } = req.params;
  // Use the Supabase function to get resources with lat/lon
  const { data, error } = await supabase.rpc('get_resources_with_geojson', { p_disaster_id: disasterId });
...
// POST /resources/:disasterId (protected)
router.post('/:disasterId', async (req, res) => {
  ...
  req.app.get('io').emit('resource_updated', { type: 'create', resource: data[0], disaster_id: disasterId });
